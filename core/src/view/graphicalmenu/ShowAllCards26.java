...
public class ShowAllCards26 implements Screen {
    ...
    public ShowAllCards26(Mola game, boolean isMute, User currentLoggedInUser, String backAddress) {
    ...
    public void render(float delta) {
        ...
        text.draw(batch, "show all cards 26 - Spells and Traps", 500, 920);
        batch.draw(backButton, 10, 10, backButton.getWidth(), backButton.getHeight());
        batch.draw(next, 800, 30, next.getWidth(), next.getHeight());
        batch.draw(pre, 700, 30, pre.getWidth(), pre.getHeight());
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Wall of Revealing Light"), 250, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Yami"), 750, 840);
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (Gdx.input.getY() > 930 - next.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 800 && Gdx.input.getX() < 800 + next.getWidth()) {
                    game.setScreen(new ShowAllCards1(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
