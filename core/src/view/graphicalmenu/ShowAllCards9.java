...
public class ShowAllCards9 implements Screen {
    ...
    public ShowAllCards9(Mola game, boolean isMute, User currentLoggedInUser, String backAddress) {
    ...
    public void render(float delta) {
        ...
        text.draw(batch, "show all cards 9 - Monsters", 500, 920);
        batch.draw(backButton, 10, 10, backButton.getWidth(), backButton.getHeight());
        batch.draw(next, 800, 30, next.getWidth(), next.getHeight());
        batch.draw(pre, 700, 30, pre.getWidth(), pre.getHeight());
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Man-Eater Bug"), 250, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Marshmallon"), 750, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Mirage Dragon"), 1250, 840);
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (Gdx.input.getY() > 930 - next.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 800 && Gdx.input.getX() < 800 + next.getWidth()) {
                    game.setScreen(new ShowAllCards10(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
            ...
            if (Gdx.input.getY() > 930 - pre.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 700 && Gdx.input.getX() < 700 + pre.getWidth()) {
                    game.setScreen(new ShowAllCards8(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
