...
public class ShowAllCards21 implements Screen {
    SpriteBatch batch;
    final Mola game;
    ...
    Texture pre;
    Texture raigeki;
    Texture ringOfDefense;
    Texture solemnWarning;
    User currentLoggedInUser;
    String backAddress;

    public ShowAllCards21(Mola game, boolean isMute, User currentLoggedInUser, String backAddress) {
    ...
    public void render(float delta) {
        ...
        text.draw(batch, "show all cards 21 - Spells and Traps", 500, 920);
        batch.draw(backButton, 10, 10, backButton.getWidth(), backButton.getHeight());
        batch.draw(next, 800, 30, next.getWidth(), next.getHeight());
        batch.draw(pre, 700, 30, pre.getWidth(), pre.getHeight());
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Raigeki"), 250, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Ring of defense"), 750, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Solemn Warning"), 1250, 840);
        batch.draw(raigeki, 100, 150, raigeki.getWidth(), raigeki.getHeight());
        batch.draw(ringOfDefense, 600, 150, ringOfDefense.getWidth(), ringOfDefense.getHeight());
        batch.draw(solemnWarning, 1100, 150, solemnWarning.getWidth(), solemnWarning.getHeight());
        batch.end();
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (Gdx.input.getY() > 930 - next.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 800 && Gdx.input.getX() < 800 + next.getWidth()) {
                    game.setScreen(new ShowAllCards22(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
            ...
            if (Gdx.input.getY() > 930 - pre.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 700 && Gdx.input.getX() < 700 + pre.getWidth()) {
                    game.setScreen(new ShowAllCards20(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
