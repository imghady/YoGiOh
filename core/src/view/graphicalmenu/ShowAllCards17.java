...
public class ShowAllCards17 implements Screen {
    SpriteBatch batch;
    final Mola game;
    ...
    BitmapFont text2;
    Texture forest;
    Texture harpiesFeatherDuster;
    Texture magicCylinder;
    User currentLoggedInUser;
    String backAddress;

    public ShowAllCards17(Mola game, boolean isMute, User currentLoggedInUser, String backAddress) {
    ...
    public void render(float delta) {
        ...
        text.draw(batch, "show all cards 17 - Spells and Traps", 500, 920);
        batch.draw(backButton, 10, 10, backButton.getWidth(), backButton.getHeight());
        batch.draw(next, 800, 30, next.getWidth(), next.getHeight());
        batch.draw(pre, 700, 30, pre.getWidth(), pre.getHeight());
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Forest"), 250, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Harpie's Feather Duster"), 750, 840);
        text2.draw(batch, "asset: " + currentLoggedInUser.showNumberOfCard("Magic Cylinder"), 1250, 840);
        batch.draw(forest, 100, 150, forest.getWidth(), forest.getHeight());
        batch.draw(harpiesFeatherDuster, 600, 150, harpiesFeatherDuster.getWidth(), harpiesFeatherDuster.getHeight());
        batch.draw(magicCylinder, 1100, 150, magicCylinder.getWidth(), magicCylinder.getHeight());
        batch.end();
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (Gdx.input.getY() > 930 - next.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 800 && Gdx.input.getX() < 800 + next.getWidth()) {
                    game.setScreen(new ShowAllCards18(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
            ...
            if (Gdx.input.getY() > 930 - pre.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 700 && Gdx.input.getX() < 700 + pre.getWidth()) {
                    game.setScreen(new ShowAllCards16(game, isMute, currentLoggedInUser, backAddress));
                    dispose();
                }
            }
