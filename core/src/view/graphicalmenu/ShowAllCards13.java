...
public class ShowAllCards13 implements Screen {
    ...
    public ShowAllCards13(MyGdxGame game, boolean isMute, User currentLoggedInUser) {
    ...
    public void render(float delta) {
        ...
        text.draw(batch, "show all cards 13 - Monsters", 500, 900);
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (Gdx.input.getY() > 930 - next.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 800 && Gdx.input.getX() < 800 + next.getWidth()) {
                    game.setScreen(new ShowAllCards14(game, isMute, currentLoggedInUser));
                    dispose();
                }
            }
            ...
            if (Gdx.input.getY() > 930 - pre.getHeight() && Gdx.input.getY() < 930) {
                if (Gdx.input.getX() > 700 && Gdx.input.getX() < 700 + pre.getWidth()) {
                    game.setScreen(new ShowAllCards12(game, isMute, currentLoggedInUser));
                    dispose();
                }
            }
